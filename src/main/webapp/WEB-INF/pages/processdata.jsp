        ...
        <div id="page-inner">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <h5>Уровень доступа: инженер по расшифровке полетной информации</h5>
                </div>
            </div>
            <!-- /. ROW  -->
            <hr/>
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <c:if test="${!empty listFlightProcessingData}">
                                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                        ...
                                        <tr>
                                            ...
                                            <th>Event 2</th>
                                            <th>Обработал(а)</th>
                                            <th>Доступно для ЛО</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        <c:forEach items="${listFlightProcessingData}" var="data">
                                            <tr class="gradeA">  <!-- HARDCODE !!! --->
                                                <td>${data.id}</td>
                                                <td><a href="/sqinfo/${data.id}" target="_blank">${data.incomingNumber}</a></td>
                                                <td>${data.dateOfProcessing}</td>
                                                <td>${data.dateOfFlight}</td>
                                                <td>${data.flightNumber}</td>
                                                <td>${data.departure}</td>
                                                <td>${data.arrival}</td>
                                                <td>${data.tailNumber}</td>
                                                <td>${data.firstEventCode}</td>
                                                <td>${data.secondEventCode}</td>
